...
if [ -z "$CITY_NAME" ]; then
    ...
    IPCURL="$(/usr/bin/curl -s "https://ipinfo.io/${IP}")"
    CITY_NAME="$(echo "${IPCURL}" | /usr/bin/jq -r ".city")"
    COUNTRY_CODE="$(echo "${IPCURL}" | /usr/bin/jq -r ".country")"
fi
...
URL="api.openweathermap.org/data/2.5/weather?appid=$APIKEY$UNIT_URL&lang=$LANG&q=$(echo "${CITY_NAME}"| /usr/bin/sed 's/ /%20/g'),${COUNTRY_CODE}"
...
function getData {
    ...
    RESPONSECODE=0
    if [ $CODE -eq 0 ]; then
      RESPONSECODE="$(echo "${RESPONSE}" | /usr/bin/jq .cod)"
    fi
    if [ "${CODE}" -ne 0 ] || [ "${RESPONSECODE:=429}" -ne 200 ]; then
        if [ $CODE -ne 0 ]; then
            ERR_MSG="/usr/bin/curl Error $CODE"
            # echo "/usr/bin/curl Error $CODE" >> "$HOME/.weather.log"
        else
...
function setIcons {
    ...
    WIND=""
    WINDFORCE=$(echo "$RESPONSE" | /usr/bin/jq .wind.speed)
    WINDICON=""
    if [ $BEAUFORTICON == "yes" ];then
        WINDFORCE2=$(echo "scale=$DECIMALS;$WINDFORCE * 3.6 / 1" | /usr/bin/bc)
    ...
    if [ $KNOTS = "yes" ]; then
        case $UNITS in
            "imperial") 
                # The division by one is necessary because scale works only for divisions. /usr/bin/bc is stupid.
                WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE * 0.8689762419 / 1" | /usr/bin/bc)
                ;;
            *)
                WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE * 1.943844 / 1" | /usr/bin/bc)
                ;;
        esac
    else
        if [ $UNITS != "imperial" ]; then
            # Conversion from m/s to km/h
            WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE * 3.6 / 1" | /usr/bin/bc)
        else
            WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE / 1" | /usr/bin/bc)
        fi
    fi
    if [ "$DISPLAY_WIND" = "yes" ] && [ $(echo "$WINDFORCE >= $MIN_WIND" |/usr/bin/bc -l) -eq 1 ]; then
    ...
    TEMP=$(echo "$TEMP" | /usr/bin/cut -d "." -f 1)
    ...
    elif [ $(echo "$TEMP >= $HOT_TEMP" | /usr/bin/bc) -eq 1 ]; then
...
getData "$1"
if [ $ERROR -eq 0 ]; then
    MAIN=$(echo "$RESPONSE" | /usr/bin/jq .weather[0].main)
    WID=$(echo "$RESPONSE" | /usr/bin/jq .weather[0].id)
    DESC=$(echo "$RESPONSE" | /usr/bin/jq .weather[0].description)
    SUNRISE=$(echo "$RESPONSE" | /usr/bin/jq .sys.sunrise)
    SUNSET=$(echo "$RESPONSE" | /usr/bin/jq .sys.sunset)
    DATE=$(/usr/bin/date +%s)
    WIND=""
    TEMP=$(echo "$RESPONSE" | /usr/bin/jq .main.temp)
    if [ $DISPLAY_LABEL = "yes" ]; then
        DESCRIPTION=$(echo "$RESPONSE" | /usr/bin/jq .weather[0].description | /usr/bin/tr -d '"' | /usr/bin/awk '{for (i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)} 1')" "
    else
        DESCRIPTION=""
    fi
    PRESSURE=$(echo "$RESPONSE" | /usr/bin/jq .main.pressure)
