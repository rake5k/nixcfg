...
''
  ...
  APIKEY="$(${pkgs.coreutils}/bin/cat "''${HOME}/.local/share/secrets/owm-key")"
  ...
  if [ -z "$CITY_NAME" ]; then
      IP="$(${getExe pkgs.curl} -s ifconfig.me)"  # == ip
      IPCURL="$(${getExe pkgs.curl} -s "https://ipinfo.io/''${IP}")"
      CITY_NAME="$(echo "''${IPCURL}" | ${getExe pkgs.jq} -r ".city")"
      COUNTRY_CODE="$(echo "''${IPCURL}" | ${getExe pkgs.jq} -r ".country")"
  fi
  ...
  URL="api.openweathermap.org/data/2.5/weather?appid=$APIKEY$UNIT_URL&lang=$LANG&q=$(echo "''${CITY_NAME}"| ${getExe pkgs.gnused} 's/ /%20/g'),''${COUNTRY_CODE}"

  function getData {
      ERROR=0
      # For logging purposes
      # echo " " >> "$HOME/.weather.log"
      # echo `${pkgs.coreutils}/bin/date`" ################################" >> "$HOME/.weather.log"
      RESPONSE="$(${getExe pkgs.curl} -s "''${URL}")"
      CODE="$?"
      if [ "$1" = "-d" ]; then
          echo "''${RESPONSE}"
          echo ""
      ...
      RESPONSECODE=0
      if [ $CODE -eq 0 ]; then
        RESPONSECODE="$(echo "''${RESPONSE}" | ${getExe pkgs.jq} .cod)"
      fi
      if [ "''${CODE}" -ne 0 ] || [ "''${RESPONSECODE:=429}" -ne 200 ]; then
          if [ $CODE -ne 0 ]; then
              ERR_MSG="${getExe pkgs.curl} Error $CODE"
              # echo "${getExe pkgs.curl} Error $CODE" >> "$HOME/.weather.log"
          else
      ...
      #     echo `${pkgs.coreutils}/bin/date +%s` >> "$HOME/.weather-last"
      fi
  }
  ...
  function setIcons {
      ...
      WIND=""
      WINDFORCE=$(echo "$RESPONSE" | ${getExe pkgs.jq} .wind.speed)
      WINDICON=""
      if [ $BEAUFORTICON == "yes" ];then
          WINDFORCE2=$(echo "scale=$DECIMALS;$WINDFORCE * 3.6 / 1" | ${getExe pkgs.bc})
      ...
      if [ $KNOTS = "yes" ]; then
          case $UNITS in
              "imperial") 
                  # The division by one is necessary because scale works only for divisions. ${getExe pkgs.bc} is stupid.
                  WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE * 0.8689762419 / 1" | ${getExe pkgs.bc})
                  ;;
              *)
                  WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE * 1.943844 / 1" | ${getExe pkgs.bc})
                  ;;
          esac
      else
          if [ $UNITS != "imperial" ]; then
              # Conversion from m/s to km/h
              WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE * 3.6 / 1" | ${getExe pkgs.bc})
          else
              WINDFORCE=$(echo "scale=$DECIMALS;$WINDFORCE / 1" | ${getExe pkgs.bc})
          fi
      fi
      if [ "$DISPLAY_WIND" = "yes" ] && [ $(echo "$WINDFORCE >= $MIN_WIND" |${getExe pkgs.bc} -l) -eq 1 ]; then
      ...
      TEMP=$(echo "$TEMP" | ${pkgs.coreutils}/bin/cut -d "." -f 1)
      ...
      elif [ $(echo "$TEMP >= $HOT_TEMP" | ${getExe pkgs.bc}) -eq 1 ]; then
  ...
  getData "$1"
  if [ $ERROR -eq 0 ]; then
      MAIN=$(echo "$RESPONSE" | ${getExe pkgs.jq} .weather[0].main)
      WID=$(echo "$RESPONSE" | ${getExe pkgs.jq} .weather[0].id)
      DESC=$(echo "$RESPONSE" | ${getExe pkgs.jq} .weather[0].description)
      SUNRISE=$(echo "$RESPONSE" | ${getExe pkgs.jq} .sys.sunrise)
      SUNSET=$(echo "$RESPONSE" | ${getExe pkgs.jq} .sys.sunset)
      DATE=$(${pkgs.coreutils}/bin/date +%s)
      WIND=""
      TEMP=$(echo "$RESPONSE" | ${getExe pkgs.jq} .main.temp)
      if [ $DISPLAY_LABEL = "yes" ]; then
          DESCRIPTION=$(echo "$RESPONSE" | ${getExe pkgs.jq} .weather[0].description | tr -d '"' | awk '{for (i=1;i<=NF;i++) $i=toupper(substr($i,1,1)) substr($i,2)} 1')" "
      else
          DESCRIPTION=""
      fi
      PRESSURE=$(echo "$RESPONSE" | ${getExe pkgs.jq} .main.pressure)
      HUMIDITY=$(echo "$RESPONSE" | ${getExe pkgs.jq} .main.humidity)
      setIcons
      outputCompact
  else
      echo " "
